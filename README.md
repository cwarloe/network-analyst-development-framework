...
**This is a work in progress. Eight of nine lessons exist.** They are listed below and you can read them right now.
...
Lessons 01, 05, 08 and 09 need nothing at all — the case is supplied in the text. Lessons 02, 03, 04 and 06 work against real packet captures in [`assets/pcaps/`](assets/pcaps/) and need Wireshark or `tshark`. Lesson 08 assumes 01–07, so it lands best with someone who already has some analysis or IT-ops background.
...
One lesson remains. **07** is unblocked and will use CIC and Stratosphere data per [R004](docs/research/R004-pcap-and-telemetry-sourcing.md).

Lesson 06 is authored but incomplete: it covers refused, dropped, reset and slow, and does **not** yet cover packet loss, latency or MTU problems, which need traffic impairment the capture lab cannot currently produce. The lesson says so itself. See the [roadmap](ROADMAP.md).
