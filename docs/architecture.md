...
- The principles and concerns below are research-supported direction combined with architectural judgment. R001–R003 support the broad direction; they do not validate this specific decomposition, and no practitioner population has confirmed its completeness.
- [R007](research/R007-nice-role-mapping.md) compared the capability model against the NICE Defensive Cybersecurity role (PD-WRL-001), using NIST's official v2.2.0 component workbook. The analytical spine corresponds — validating alerts, distinguishing benign from malicious, determining cause, evaluating sources, correlating, communicating. Roughly forty of that role's eighty-one task and skill statements have no NADF counterpart. Three clusters are **deliberate exclusions**: signature and detection work, malware handling, and incident resolution. Two are **undeclared**: vulnerability management, and security control and product evaluation. One is a **structural gap** rather than a scope choice: trend, posture and continuous-monitoring work, which the role performs across time and every NADF lesson performs at a single moment.
- Conversely, NADF's LR domain, its calibrated-confidence and stopping-point statements, and the observation/interpretation/judgment separation have **no counterpart in that role standard at all**. They are NADF's thesis rather than role coverage, and their basis labels may be generous.
