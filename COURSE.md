...
The first new Book 1 bridge is also runnable: **[Predict Before You Look](lessons/predict-before-you-look.md)** follows lesson 02 and develops IR-3, LR-1, and LR-2 through a staged prediction-and-revision record.
...
- **Guidance fades, inside each lesson as well as across them.** Baseline principle 5. Across the course, 01–04 are worked examples and guided practice, 05–07 move to guided-independent, and 08–09 are independent. Within each lesson the same ramp repeats in three rungs: a **worked example** shown in full, a **guided practice** rung with prompts and no answer key, then the **work product** unaided. [R005](docs/research/R005-instructional-design-review.md) flagged that fading only at lesson boundaries leaves the right fade point undetectable, since it differs by audience; the middle rung is where a learner finds out where they actually are.
- **The middle rung holds deep structure constant and varies surface.** The guided rung rehearses the same move as the worked example on deliberately unlike material — lesson 01 goes from an exfiltration alert to a slow intranet, lesson 05 from a data-egress question to a deny-only firewall log. Same-surface practice teaches learners to match phrasing rather than recognize the pattern. Where a lesson runs on a capture and there is no second case in the data, the rung asks a different question of the same data — predict before you look, or say what the log alone cannot establish.
