...
The gate has already earned itself twice. It caught a `403` response advertising `Content-Length: 43` for a 44-byte body, and it caught plaintext HTTP mapped to port 443, which made Zeek run the SSL analyzer and log a protocol violation that was an artefact of the anonymisation rather than anything in the traffic. Both would have shipped as "looks fine in Wireshark."
