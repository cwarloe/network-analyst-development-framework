...
- ~~**Revisit the basis labels** on LR-1 to LR-4 and JU-2/JU-4.~~ **Withdrawn** — the recommendation applied a role-standard test to a research-basis label, and the LR cluster turns out to be among the *best*-supported statements in the model, tracing to a sourced finding with a Directly Supported trace. JU-4 was already `AJ`. Reasons recorded in [R007](docs/research/R007-nice-role-mapping.md). What survives is a different observation: 23 of 27 statements carry `RS/AJ`, so the label barely discriminates. Fixing that means re-deriving every basis, not downgrading six.
